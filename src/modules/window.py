...
class Window:
    def __init__(self, 
        ...
        if fullscreen:
            ...
            self.window: glfw._GLFWwindow = glfw.create_window(self.width,
                                                               self.height,
                                                               "Main",
                                                               self.monitor,
                                                               None)
    ...
    def __key_callback(self, window: glfw._GLFWwindow, key, scancode, action, mods) -> None:
        ...
        if self.keys[glfw.KEY_A]:
            self.scene.camera.position -= glm.normalize(
                glm.cross(self.scene.camera.target, self.scene.camera.up)
                ) * camera_speed
    ...
    def __mouse_callback(self, window: glfw._GLFWwindow, xpos: float, ypos: float):
        front = glm.vec3(0)
        ...
        front.x = glm.cos(glm.radians(self.scene.camera.yaw)) * glm.cos(glm.radians(self.scene.camera.pitch))
        front.y = glm.sin(glm.radians(self.scene.camera.pitch))
        front.z = glm.sin(glm.radians(self.scene.camera.yaw)) * glm.cos(glm.radians(self.scene.camera.pitch))
            
        self.scene.camera.target = glm.normalize(front)

    def __scroll_callback(self, window: glfw._GLFWwindow, xoffset: float, yoffset: float):
        if self.scene.fov >= 1.0 and self.scene.fov <= 90.0:
            self.scene.fov -= yoffset
        self.scene.fov = 1.0 if self.scene.fov < 1.0 else \
            90.0 if self.scene.fov > 90.0 else \
                self.scene.fov
